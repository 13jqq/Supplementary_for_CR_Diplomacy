...
def version_with_ablation(args) -> str:
    version_tag = normalize_version_tag(args.version)
    abl = str(args.ablation).strip().lower()
    if abl == "full":
        return version_tag
    return f"{version_tag}_{abl}"


def version_with_ablation_lower(args) -> str:
    return version_with_ablation(args).lower()
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help=(
            "Ablation setting passed to ConsistentAgent: "
            "full, no_c12, no_c34, no_all."
        ),
    )
...
def build_root_dir(args, agents: List[str]) -> Path:
    version_tag = version_with_ablation(args)
    if args.log_root:
...
def iter_tasks(args, agents: List[str], root_dir: Path):
    for block_seed in range(args.seed_start, args.seed_end + 1):
        ...
        for shift, assignment in enumerate(assignments):
            ...
            log_path = log_path_for(root_dir, block_seed, shift, game_seed, version_with_ablation(args))
...
def find_and_claim_next_task(args, agents: List[str], root_dir: Path):
    for task in iter_tasks(args, agents, root_dir):
        ...
        payload = {
            ...
            "version": version_with_ablation(args),
            "source": args.source,
            "mode": args.mode,
            "topk": args.topk,
            ...
            "run_version": version_with_ablation(args),
        }
...
def write_assignment_plan(args, agents: List[str], root_dir: Path):
    ...
    plan_path = root_dir / f"assignment_plan_{version_with_ablation_lower(args)}.jsonl"
